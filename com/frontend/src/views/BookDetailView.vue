  ...
  <div class="page-container py-8">
    ...
    <div v-else-if="book">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <!-- 가격 비교 (신간 서점 + Y-Danawa 중고 마켓 통합 그리드) -->
        <div class="card p-5 md:col-span-2">
          ...
          <div v-if="pricesLoading && usedOffersLoading" class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div v-for="n in 4" :key="n" class="skeleton h-24 rounded-xl"></div>
          </div>
          <div v-else class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <!-- Y-Danawa 중고 마켓 카드 -->
            <div v-if="usedOffersLoading" class="skeleton h-24 rounded-xl"></div>
            <component
              :is="usedOffers.length > 0 ? 'RouterLink' : 'div'"
              v-else
              :to="usedOffers.length > 0 ? { name: 'market-detail', params: { id: usedOffers[0].id } } : null"
              class="block border rounded-xl p-4 transition-colors group relative overflow-hidden"
              :class="usedOffers.length > 0 ? 'border-indigo-200 hover:border-indigo-400 bg-indigo-50/30' : 'border-gray-200 bg-gray-50/50'"
            >
              <div class="absolute top-0 right-0 px-2 py-0.5 bg-yju text-white text-[9px] font-bold rounded-bl-lg">학우중고</div>
              <p class="text-sm font-bold transition-colors" :class="usedOffers.length > 0 ? 'text-indigo-600' : 'text-gray-500'">Y-Danawa 중고</p>
              
              <template v-if="usedOffers.length > 0">
                <p class="text-xl font-black text-gray-900 mt-1">
                  {{ usedOffers[0].priceWon?.toLocaleString() }}<span class="text-sm font-normal ml-0.5">원</span>
                </p>
                <p class="text-[10px] text-indigo-500 mt-0.5 font-medium">교내 최저가!</p>
              </template>
              <template v-else>
                <p class="text-sm text-gray-400 mt-2 font-medium">판매 대기 중</p>
                <p class="text-[10px] text-gray-300 mt-0.5">첫 매물을 올려보세요</p>
              </template>
            </component>
          ...
          <div v-if="!pricesLoading && !usedOffersLoading && filteredPrices.length === 0 && usedOffers.length === 0" class="text-sm text-gray-400 text-center py-4">
            가격 정보를 불러올 수 없습니다.
          </div>
        </div>
      </div>
      ...
      <div class="card p-5 mt-4">
        ...
        <div v-else class="space-y-3 mb-4">
          <div v-for="r in comments" :key="r.id" class="border border-gray-200 rounded-lg p-3">
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] text-gray-400">{{ formatCommentDate(r.createdAt) }}</span>
              </div>
              <button
                v-if="r.ownedByMe"
...
async function submitComment() {
  ...
  try {
    await api.createReview(isbn13, content)
    commentInput.value = ''
...
async function toggleLike(comment) {
  ...
  try {
    const { data } = await api.toggleReviewLike(isbn13, comment.id)
...
async function deleteComment(commentId) {
  ...
  try {
    await api.deleteReview(isbn13, commentId)
